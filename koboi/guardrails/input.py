...
class InputGuardrail(PatternGuardrail):
    ...
    PATTERNS: list[tuple[str, str]] = [
        ...
        (r"(?i)disregard\s+(all|any|previous|the).{0,30}(instruction|prompt|rule)", "Possible prompt injection: disregard instructions"),
        (r"(?i)\bnew\s+(instructions?|rules?|prompt)\s*:", "Possible prompt injection: new instructions override"),
        (r"(?i)override\s+(your|the|all).{0,20}(instruction|prompt|rule|directive)", "Possible prompt injection: override instructions"),
        (r"(?i)you\s+are\s+now\s+", "Possible prompt injection: persona override"),
        # --- Instruction-override commands (Bahasa Indonesia) ---
        (r"(?i)abaikan\s+.*(instruksi|prompt|aturan|perintah|pesan\s+(di\s+)?atas)", "Possible prompt injection: abaikan instruksi (ID)"),
        (r"(?i)lupakan\s+(semua|sebelumnya|instruksi|prompt|aturan)", "Possible prompt injection: lupakan (ID)"),
        (r"(?i)(sekarang|mulai\s+sekarang)\s+(kamu|anda|kau)\s+(adalah|jadi)", "Possible prompt injection: persona override (ID)"),
        ...
        (r"(?i)^#{1,3}\s*(system|instructions?|prompt|rules?)\b", "Possible prompt injection: markdown role header"),
    ]
