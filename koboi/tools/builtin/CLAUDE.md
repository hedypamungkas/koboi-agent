...
## What this is
The 18 shipped tool modules, each a `@tool()`-decorated function in its own module and registered by
...
bitbucket.py    bitbucket_create_pr / bitbucket_update_pr (DESTRUCTIVE, idempotent=False) + bitbucket_list_prs / bitbucket_get_pr / bitbucket_get_default_reviewers (SAFE, read-only)   Bitbucket Cloud PR tooling; mirrors github.py (in-process httpx + HTTP Basic auth; needs `bitbucket:` config + `bitbucket_client` dep; app password never rides subprocess env)
