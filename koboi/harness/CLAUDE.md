...
- **Block-list WINS over allow-list in env filtering.** `KOBOI_DB_TOKEN` matches the `KOBOI_*` allow-glob but is still stripped by `*_TOKEN`. Order: passthrough check -> allow-list keep -> block-list drop. Plural globs (`*_KEYS`/`*_TOKENS`/`*_SECRETS`/`*_API_KEYS`) exist because `*_KEY` alone missed `KOBOI_API_KEYS` (issue #99).
- **`AUTH_BOUNDARY_BLOCKLIST` is stripped even under passthrough.** `KOBOI_API_KEYS` / `KOBOI_API_KEYS_FILE` hold (or point at) the server's OWN Bearer keys -- a subprocess that reads them can authenticate to the REST API as any tenant, so `env_passthrough: true` / `KOBOI_ENV_PASSTHROUGH=1` restore every other secret but never these two. Scope: only spawn sites that call `build_safe_env`/`sandbox.build_env` (shell, git, skills, both sandbox backends); the stdio MCP client (`koboi/mcp/client.py`) still spawns servers with the inherited env, and `submit_background_shell` falls back to the inherited env if no sandbox dep is wired.
