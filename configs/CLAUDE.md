...
web_brave.yaml            Brave Search provider demo (`web.search.provider: brave`)
web_firecrawl.yaml        Firecrawl search+fetch provider demo (`web.search.provider: firecrawl`)
...
## Top-level sections
`agent`, `mode`, `llm`, `providers`, `pools`, `tools`, `context`, `rag`, `embedding`, `guardrails`, `tracing`, `harness`, `policy`, `skills`, `mcp`, `memory`, `subagent`, `orchestration`, `sandbox`, `journal`, `server`, `jobs`, `hooks`, `eval`, `keybindings`, `web`, `research`
