...
a2a_docker_x.yaml          Cross-instance A2A -- Docker-networked instance X (see `docker-compose.a2a.yml`)
a2a_docker_y.yaml          Cross-instance A2A -- Docker-networked instance Y (see `docker-compose.a2a.yml`)
aegis_ops_full.yaml       "Full sample" -- exercises 29 of 32 KoboiConfig top-level sections (all but `research`/`keybindings`/`replay`, mutually exclusive with the DAG structure) in one DAG-orchestrated customer-ops scenario (providers/pools failover, RAG+rerank, proactive memory, self_healing, handover, restricted sandbox, MCP, media, peers, server/jobs, hooks, eval); see `examples/39_aegis_ops_full_demo.py` for the (also documented) orchestration-mode feature-coverage gaps it surfaced
```
...
## Top-level sections
`agent`, `mode`, `llm`, `providers`, `pools`, `tools`, `context`, `rag`, `embedding`, `guardrails`, `tracing`, `harness`, `policy`, `skills`, `mcp`, `memory`, `subagent`, `orchestration`, `sandbox`, `journal`, `server`, `jobs`, `hooks`, `eval`, `keybindings`, `websearch`, `research`, `handover`, `peers`, `media`, `self_healing`, `github`, `bitbucket`, `replay`
...
- `guardrails.input.deflection_text` — opt-in: when set, an `InputGuardrail` injection-pattern block surfaces this text as a graceful successful reply instead of raising `AgentGuardrailError` (`AgentGuardrailError.is_graceful_deflection`, handled in `loop.py`). Blank/whitespace normalizes to `None` (feature stays off). Empty/length input blocks never deflect.
...
- **Coding autonomy (Wave 0-4)** knobs:
  - `github:` — `{enabled, token}` (root-level; `${GITHUB_TOKEN:}`); in-process httpx for `github_*` PR tools; bypasses sandbox network tiers like `web_fetch`. Token never rides subprocess env.
  - `agent.background_shell` — `{enabled, max_lifetime_seconds: 1800}`; `submit_background_shell` runs outside the approval/policy pipeline once started (the lifetime cap is the control); in-memory job registry, not durable across restart.
  - `agent.max_total_tokens` / `max_cost_usd` / `token_prices` — per-run budget ceiling → `AgentBudgetExceededError`.
  - `agent.parallel_tools` — run independent tool calls concurrently.
  - `journal.checkpoint` — shadow-repo workdir checkpoints that roll back an interrupted non-idempotent tool call on resume.
  - `context.strategy: coding` — body-eviction strategy for long coding runs.
  - `sandbox.network: allowlist` + `sandbox.network_allowlist` (host-globs) — Wave 3 egress tier: scanned binaries may only reach allowlisted hosts (userinfo-aware host extraction). `policy.allow_interpreter_exec` governs interpreter egress.
