...
CASES: list[tuple[str, str, str, dict | None, dict, int]] = [
    ...
    (
        ...
        "/v1/sessions",
        None,
        {"Content-Type": "application/json", "Authorization": "Bearer wrong_key_xyz"},
        401,
    ),
    (
        "auth_bearer_no_space",
        "POST",
        "/v1/sessions",
        None,
    ...
    (
        "auth_lowercase_scheme",
        "POST",
        "/v1/sessions",
        None,
        {"Content-Type": "application/json", "Authorization": f"bearer {API_KEY or 'x'}"},
        401,
    ),
    ...
    (
        "path_traversal_space",
        "POST",
        "/v1/chat/stream",
        {"message": "hi"},
        {"X-Session-Id": "bad session", **_auth()},
        400,
    ),
    (
        "path_traversal_dotdot_only",
        "POST",
        "/v1/chat/stream",
        {"message": "hi"},
        {"X-Session-Id": "..", **_auth()},
    ...
    (
        "approve_unknown_session",
        "POST",
        "/v1/sessions/nonexistent-sess-3/approve",
        {"approval_id": "nonexistent-approval", "decision": "approve"},
        _auth(),
        404,
    ),
...
@pytest.mark.parametrize("method,path,body,headers,expected", [c[1:] for c in CASES], ids=[c[0] for c in CASES])
async def test_security_edge(client, method, path, body, headers, expected):
    ...
    assert r.status_code == expected, f"{method} {path}: expected {expected}, got {r.status_code}. body={r.text[:300]}"
