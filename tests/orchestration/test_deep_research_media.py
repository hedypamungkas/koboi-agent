...
Drives ``Orchestrator._run_deep_research`` with a mock client + mock media backend and asserts the
full integration: research nodes get ``generate_image`` (capabilities), a node emitting a
``generate_image`` tool_call surfaces a ``MediaGeneratedEvent``, and the post-synthesis
