...
- `bitbucket.py` -- Bitbucket Cloud PR tooling: `bitbucket_create_pr`/`bitbucket_update_pr` (DESTRUCTIVE, idempotent=False) + `bitbucket_list_prs`/`bitbucket_get_pr`/`bitbucket_get_default_reviewers` (SAFE, read-only, on the chat/plan allowlist). Mirrors `github.py`: in-process `httpx` client with HTTP Basic auth (needs `bitbucket: {enabled, username, app_password}`); bypasses sandbox network tiers (like `web_fetch`); the app password is config-supplied and must never ride subprocess env.
